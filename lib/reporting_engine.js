...
export async function generateWeeklyReports() {
    ...
    // 1. Log Cron Start
    const [cronLog] = await safeInsert('cron_execution_logs', {
        job_name: 'weekly_parent_report',
        status: 'running'
    }, {
        route: 'reporting_engine/generateWeeklyReports',
        userId: 'system',
    });
    ...
    try {
        ...
        if (users && users.length > 0) {
            for (const user of users) {
                ...
                const [report] = await safeInsert('weekly_parent_reports', {
                    user_id: user.id,
                ...
                if (report) {
                    // Queue for delivery
                    await safeInsert('parent_report_logs', {
                        report_id: report.id,
        ...
        if (pendingLogs && pendingLogs.length > 0) {
            for (const log of pendingLogs) {
                ...
                await safeUpdate('parent_report_logs', { id: log.id }, {
        ...
        if (cronLogId) {
            await safeUpdate('cron_execution_logs', { id: cronLogId }, {
                status: 'success',
