...
export async function bootApp() {
    if (booted) return; // Idempotent — safe to call multiple times
    booted = true;
...
async function restoreLocalState() {
    // Restore any pending offline test state from IndexedDB
    // (existing autosave restoration logic integrates here)
    if (typeof indexedDB === 'undefined') return;
    // No-op if no pending state — safe to call always
}

async function replayOfflineQueue() {
    // Replay any queued API calls (test submissions, answers) accumulated offline
