...
const SCHEMAS = {
    ...
    PURCHASE_SUBSCRIPTION: { required: ['planId'] },
    ACKNOWLEDGE_PURCHASE: { required: ['purchaseToken'] },
...
export function purchaseNativeSubscription(planId) {
    if (supportsCapability('purchaseSubscription')) {
        return postNativeIntent('PURCHASE_SUBSCRIPTION', { planId }, 5 * 60 * 1000);
    }
    return Promise.reject(new Error('PLAY_BILLING_UNAVAILABLE'));
}

export function acknowledgeNativePurchase(purchaseToken) {
    if (supportsCapability('purchaseSubscription')) {
        return postNativeIntent('ACKNOWLEDGE_PURCHASE', { purchaseToken }, 15_000);
    }
    return Promise.reject(new Error('PLAY_BILLING_UNAVAILABLE'));
}
