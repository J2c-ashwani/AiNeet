...
const genAI = process.env.GEMINI_API_KEY && process.env.GEMINI_API_KEY !== 'mock-key'
...
const NEET_SYSTEM_PROMPT = `You are a strict, expert NEET (National Eligibility cum Entrance Test) coach and subject matter expert in Physics, Chemistry, and Biology.
...
async function callGroq(userPrompt) {
    ...
    if (!res.ok) {
        ...
        throw new Error(`GROQ_HTTP_${res.status}: ${errBody}`);
    }
...
async function callOpenRouter(userPrompt) {
    ...
    if (!res.ok) {
        const errBody = await res.text().catch(() => 'Unknown error');
        throw new Error(`OPENROUTER_HTTP_${res.status}: ${errBody}`);
    }
...
async function callAIWithFallback(fullPrompt, userQuestion) {
    const providers = [
        { name: 'gemini', fn: () => callGemini(fullPrompt) },
        { name: 'groq',   fn: () => callGroq(userQuestion) },
        { name: 'openrouter', fn: () => callOpenRouter(userQuestion) },
    ];
...
export async function generateDoubtResponse(question, context = '', user = null) {
    ...
    // 1.5 Pre-flight Semantic Guardrails to prevent abuse
    ...
    // If no AI keys at all, use mock
    if (!process.env.GEMINI_API_KEY && !process.env.GROQ_API_KEY && !process.env.OPENROUTER_API_KEY) {
        console.warn('No AI API keys found. Using fallback mock response.');
    ...
    try {
        // Build the full prompt (used by Gemini; Groq uses system prompt separately)
        const fullPrompt = `${NEET_SYSTEM_PROMPT}
...
Provide a detailed, easy-to-understand explanation following the structure specified.`;

        // Call AI with automatic fallback: Gemini → Groq
        const { text, provider } = await callAIWithFallback(fullPrompt, 
            `${question}${context ? `\n\nContext: ${context}` : ''}`);
