...
export async function generateDoubtResponse(question, context = '', user = null) {
    ...
    const { data: cached } = await supabase.from('ai_response_cache').select('response').eq('request_hash', requestHash).maybeSingle();
    if (cached) {
    ...
    try {
        ...
        const prompt = `
        You are a strict, expert NEET (National Eligibility cum Entrance Test) coach and subject matter expert in Physics, Chemistry, and Biology.
        ...
        If the doubt is within the NEET syllabus, provide a detailed, easy-to-understand explanation following this structure:
        ...
        // Cache result
        try {
            await supabase.from('ai_response_cache').insert({ request_hash: requestHash, response: text });
        } catch (e) {
