...
export async function analyzeImageDoubt(userId, userTier, imageBuffer, mimeType) {
    try {
        ...
        const canProceed = await UsageTracker.checkLimit(userId, userTier, 'doubt');
        if (!canProceed.allowed) throw new Error(canProceed.message || "Daily doubt limit reached.");

        if (!genAI) {
            throw new Error('Vision AI is temporarily unavailable.');
        }
        ...
        await UsageTracker.incrementUsage(userId, 'doubt', 0);
