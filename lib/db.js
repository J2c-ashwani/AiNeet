...
if (isPostgres) {
  // Parse DATABASE_URL to handle passwords with special chars (like @ in Ashwani@7903)
  // We use lastIndexOf('@') to split user:pass from host since @ can appear in password
  const dbUrl = process.env.DATABASE_URL;
  const protocolEnd = dbUrl.indexOf('://') + 3;
  const lastAt = dbUrl.lastIndexOf('@');
  const userPass = dbUrl.substring(protocolEnd, lastAt);
  const hostPart = dbUrl.substring(lastAt + 1); // host:port/database
