...
function localCashfreeMockAllowed() {
    return process.env.NODE_ENV !== 'production' && process.env.CASHFREE_ALLOW_MOCK === 'true';
}

function assertCashfreeConfigured() {
    if (!hasMockCashfreeConfig()) return false;
    if (localCashfreeMockAllowed()) return true;
    throw new Error('Cashfree is not configured. Refusing to use mock payments.');
}
...
export const PaymentService = {
    ...
    async createOrder(amount, currency = 'INR', customer = {}, planId = '') {
        // If Cashfree is not configured or uses dummy keys, return mock
        if (assertCashfreeConfigured()) {
            console.warn('⚠️ Cashfree not configured — using mock payment');
            return {
                orderId: `mock_order_${randomUUID().slice(0, 8)}`,
                paymentSessionId: `mock_session_${randomUUID().slice(0, 8)}`,
                orderStatus: 'ACTIVE',
                isMock: true,
            };
        }
        ...
        const orderPayload = {
            ...
            order_meta: {
                return_url: `${process.env.NEXT_PUBLIC_APP_URL || 'https://ai-neet.vercel.app'}/profile?payment=success&order_id={order_id}`,
