...
const CASHFREE_ENV = (process.env.CASHFREE_ENV || '').toLowerCase();
const inferredCashfreeEnv = CASHFREE_SECRET_KEY?.includes('_prod_') ? 'production' : 'sandbox';
const cashfreeEnvironment = CASHFREE_ENV || inferredCashfreeEnv;
if (!['production', 'sandbox'].includes(cashfreeEnvironment)) {
    throw new Error('CASHFREE_ENV must be either "production" or "sandbox".');
}

// Provider environment is explicit. NODE_ENV must never decide where money flows.
