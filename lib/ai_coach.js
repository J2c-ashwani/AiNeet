 ...
 * @param {string} userId
 */
export function generateDailyGuidance(userId) {
    const db = getDb();
    const user = db.prepare('SELECT name, streak, last_active_date, xp FROM users WHERE id = ?').get(userId);
