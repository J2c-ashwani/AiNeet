...
export async function checkFeatureAccess(userId, feature, upgradeTo = 'pro') {
    ...
    const { data: user } = await supabase.from('users').select('subscription_tier, subscription_status, subscription_expiry').eq('id', userId).single();
...
export async function checkUsageLimit(userId, feature, currentUsage = 0) {
    const supabase = await getDb();
    const { data: user } = await supabase.from('users').select('subscription_tier, subscription_expiry').eq('id', userId).single();
