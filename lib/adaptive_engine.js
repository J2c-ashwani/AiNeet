...
export async function updateQuestionDifficulty(questionId, isCorrect, userMastery) {
    ...
    if (!qStats) {
        ...
        await safeInsert('question_difficulty_dynamic', {
            question_id: String(questionId),
            difficulty_score: DEFAULT_RATING
        }, {
            route: 'adaptive_engine/updateQuestionDifficulty',
        });
    ...
    await safeUpdate('question_difficulty_dynamic', { question_id: String(questionId) }, {
...
export async function updateUserMastery(userId, topicId, isCorrect, questionDifficulty) {
    ...
    if (!uStats) {
        await safeInsert('user_topic_mastery', {
            user_id: userId,
            topic_id: String(topicId),
            mastery_score: DEFAULT_RATING
        }, {
            route: 'adaptive_engine/updateUserMastery',
            userId,
        });
    ...
    await safeUpdate('user_topic_mastery', { user_id: userId, topic_id: String(topicId) }, {
...
export async function getAdaptiveQuestion(userId, subjectId, topicId = null, excludeIds = []) {
    ...
    if (topicId) {
        // Wave 5: Harm Prevention (P0)
        // Check if this weak topic has been punitively drilled too much today
        const todayStr = new Date().toISOString().split('T')[0];
        const { count: dailyTopicNudges } = await supabase.from('recommendation_explanations')
            .select('*', { count: 'exact', head: true })
            .eq('user_id', userId)
            .eq('recommended_topic_id', topicId)
            .gte('created_at', todayStr);
            
        if (dailyTopicNudges >= 3) {
            // Harm prevention: block doom-loops. Force general subject recommendation instead.
            console.warn(`[Harm Prevention] User ${userId} trapped in topic ${topicId} loop. Reverting to subject-level adaptive.`);
            topicId = null; 
        }
    ...
    if (validQs.length > 0) {
        ...
        // Wave 5: Explainability Logging & Decision Hashing (P0/P1)
        const engineVersion = 'v2.0_governed';
        const rawPayload = {
            ...
            pool_size: validQs.length
        };
        
        // Generate cryptographic hash of the exact state for forensic replayability
        const hashPayload = `${userId}:${topicId}:${targetRating}:${idealDiff}:${best.id}:${engineVersion}`;
        const decisionHash = crypto.createHash('sha256').update(hashPayload).digest('hex');
        ...
        supabase.from('recommendation_explanations').insert({
            ...
            explanation_payload: rawPayload,
