...
export async function updateQuestionDifficulty(questionId, isCorrect, userMastery) {
    ...
    let qStats = await db.get('SELECT difficulty_score, attempts, correct_attempts FROM question_difficulty_dynamic WHERE question_id = ?', [questionId]);

    if (!qStats) {
        // Initialize if not exists
        await db.run('INSERT INTO question_difficulty_dynamic (question_id, difficulty_score) VALUES (?, ?)', [questionId, DEFAULT_RATING]);
...
export async function updateUserMastery(userId, topicId, isCorrect, questionDifficulty) {
    ...
    if (!uStats) {
        await db.run('INSERT INTO user_topic_mastery (user_id, topic_id, mastery_score) VALUES (?, ?, ?)', [userId, topicId, DEFAULT_RATING]);
