...
async function validateQuestion(rawQuestion, { crossVerifyFn, dedupFn, topicId }) {
    try {
        ...
        // Check if it eliminates wrong options (e.g., mentions option_a or option B)
        // A simple heuristic: checking if option strings appear in explanation (might be too strict for math, but good for bio)
        qualityScore += 10; // Base points for structure
        
        // Penalize generic explanations
        if (expl.toLowerCase().includes('the correct answer is') && expl.length < 100) {
            qualityScore -= 50;
        }
